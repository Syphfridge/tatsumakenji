// Supply & Demand Simulation (Polished + Layout Fixed)
...
// === UTILS FOR RESPONSIVE CANVAS ===
function getCanvasWidth() {
  // Up to 1280px, but also shrinks if window is smaller
  return min(windowWidth - 40, 1280);
}
...
function setup() {
  ...
  // --- Position elements (under the canvas) ---
...
function draw() {
  ...
  let cPolicy = cBase + tax - subsidy;   // policy supply intercept
...
// === Handle window resizes (keep width responsive) ===
...
function drawCurves(aBase, b, cBase, d, cPolicy, tax, subsidy) {
  ...
  if (tax > 0 || subsidy > 0) {
    ...
    // No policy: show base as solid red
    stroke(255, 80, 80);
    strokeWeight(2);
    last = null;
    for (let Q = 0; Q <= Qmax; Q += 0.1) {
...
function drawParameterText(
  ...
  // move panel left a bit & wrap text so nothing is cut
  let x0 = width - 380;
  let y0 = margin;
  let maxW = width - x0 - 10;
  ...
  text(
    `Demand: P = (${a.toFixed(1)} + shift Demand) - ${b.toFixed(2)}Q`,
    x0,
    y0,
    maxW,
    40
  ...
  text(
    `Supply: P = (${c.toFixed(1)} + shift Supply + tax - subsidy) + ${d.toFixed(
      2
    )}Q`,
    x0,
    y0 + 18,
  ...
  text(
    `shift Demand = ${dShift.toFixed(1)}, shift Supply = ${sShift.toFixed(
      1
...
function resetDefaults() {
  ...
  // do NOT force scenario here, so dropdown choice sticks
}
...
function applyScenario() {
  ...
  // keep the dropdown on the selected scenario
